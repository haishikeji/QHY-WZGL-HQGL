...
@Slf4j
@Component
public class BzrbJobTimerTaskRunner implements CommonTimerTaskRunner {
...
//        cgJhSpckmxQueryWrapper.notIn("cklx","商品退货");
        ...
        cgJhSpckmxQueryWrapper.gt("ckrq","2024-08-01");
        cgJhSpckmxQueryWrapper.lt("ckrq","2024-09-01");
        cgJhSpckmxQueryWrapper.notIn("cklx","商品退货");
        ...
        cgJhSpckmxQueryWrapper.notInSql("id"," select id from  (select * from cg_jh_spckmx where  ckrq>'2024-07-01' and ckrq<'2024-08-01' and bmdm in ('0401','0402','0404','0410','0411','0413','0414','0415')  and cklx not in('商品退货'))a where id in(select ckid from biz_spt_ckd )");
        cgJhSpckmxQueryWrapper.select("id, (case when cklx!='库存退库' then cksl else -cksl end)cksl,(select name from sys_org where id=cg_jh_spckmx.bmdm)bmmc,(select spcode from biz_spt_spdm where spid=(select id from cg_kc_spdm where cg_jh_spckmx.spdm=spdm))spdm, (case when cklx!='库存退库' then je else -je end)je, gydj,spjhrq,ckrq");
...
String json=JSONUtil.toJsonStr(body);
    ...
    private void genBzrb(){
        ...
        orgList.forEach(bizOrg -> {
            ...
            cgZwBzrbService.genBzrbAuto( cgZwBzrbAddParam);
        });
    }
}
