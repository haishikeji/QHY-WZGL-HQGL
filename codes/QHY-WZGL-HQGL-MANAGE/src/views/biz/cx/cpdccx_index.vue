	...
	<a-card :bordered="false">
		<a-form ref="searchFormRef" name="advanced_search" :model="searchFormState" class="ant-advanced-search-form">
			<a-row :gutter="24">
				...
				<a-col :xxl="6" :xl="6" :lg="8" :md="12" :sm="24">
					<a-form-item label="班组或部门" name="workstate">
						<a-select v-model:value="searchFormState.org" :defaultValue="dws">
							<a-select-option
...
<!--				<template v-if="column.dataIndex === 'cksl'">-->
<!--					<div>-->
<!--						<a-input-number-->
<!--							:min="0"-->
<!--							v-model:value="record.cksl"-->
...
let searchFormState = reactive({cglx:'成品调拨',workstate:'已提交',org:'班组',gysdm:userInfo.value.orgId})
...
const bzInfo = ref([])
const gysInfo = ref([])
const workstate = ref(['已提交','收货中','已收货'])
const org = ref(['班组','部门'])
const isDzc = ref(['是','否'])
const dws = ref('已提交')
...
const loadData = (parameter) => {
	...
	if (searchFormParam.org === '班组'){
		return cgJhSpmxApi.cgJhSpckmxCpdbPagebz(Object.assign(parameter, searchFormParam)).then((data) => {
			let allData = {
				"records": data
...
const acceptBatchCgJhSpckmx = (params) => {
	cgJhSpmxApi.acceptBatchCgJhSpckmxCpdb(params).then(() => {
