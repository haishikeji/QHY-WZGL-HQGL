				...
				<a-row :gutter="24">
					...
					<a-col :xxl="6" :xl="6" :lg="8" :md="12" :sm="24">
						...
						<a-form-item label="班组名称" name="ffbz">
							<a-select v-model:value="searchFormState.ffbz" placeholder="请输入班组"
									  :options="bzInfo">
			...
			>
				...
				<template #bodyCell="{ column, record }">
					<template v-if="column.dataIndex === 'action'">
						<a-space>
							<a-popconfirm title="确定要撤回吗？" @confirm="deleteCgLyMx(record)"
										  v-if="record.workstate=='申请中'">
...
let searchFormState = reactive({ isZero: "false", cklx: "库存领用", workstate: "申请中" });
...
const deleteCgLyMx = (params) => {
	spinning.value = true;
	cgJhSpmxApi.deleteCgLyMx(params).then(() => {
		spinning.value = false;
		table.value.clearRefreshSelected();
	});
};
