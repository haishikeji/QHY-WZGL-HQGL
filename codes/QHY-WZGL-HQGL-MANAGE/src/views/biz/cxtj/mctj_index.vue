<template>
	<a-card :bordered="false">
		...
		<s-table
			...
			:scroll="{ x: 'calc(100vw - 350px)', y: 'calc(100vh - 350px)' }"
		>
			<!-- <template #operator class="table-operator">
                <a-space>
					...
					<a-button type="primary" @click="printByQuantity()" >
						<template #icon><printer-outlined /> </template>
						按数量统计打印
					</a-button>
                </a-space>
...
<script setup name="kckczb">
	...
	//let searchFormState = reactive({cklx:"库存领用"})
	...
	const loadData = (parameter) => {
		...
		return cgJhSpmxApi.mctjPage(Object.assign(parameter, searchFormParam)).then((data) => {
			return data
		})
	}
	// 重置
	...
	const print = () => {
		if (!searchFormState.shrq) {
			...
			let spdm = searchFormState.spdm ? searchFormState.spdm : ''
			visible.value = true
			src.value =
				'https://hqncg.just.edu.cn/decision/view/report?viewlet=cgjkd%252Fcwtj%252Fspmctj.cpt&bmdm=' +
				bmdm +
				'&lbdm=' +
				lbdm +
				'&spdm=' +
	...
	const printByQuantity = () => {
		if (!searchFormState.shrq) {
			...
			visible.value = true
			src.value =
				'https://hqncg.just.edu.cn/decision/view/report?viewlet=cgjkd%252Fcwtj%252Fspmctjasl.cpt&bmdm=' +
				bmdm +
				'&lbdm=' +
				lbdm +
				'&spdm=' +
