            ...
            <a-row :gutter="16">
                <a-col :span="24">
                    <a-form-item label="申请单号" name="sqdh">
                        <a-input v-model:value="formData.sqdh" placeholder="请输入申请单号" allow-clear readonly="readonly"/>
                    </a-form-item>
                </a-col>
			</a-row>
			<a-row :gutter="16">
...
<script setup name="cgJhSpmxForm">
    ...
    const onOpen = (record) => {
        ...
        if (record) {
			...
			formData.value.xhrq = dayjs(new Date().getTime() + 24 * 60 * 60 * 1000,"YYYY-MM-DD HH:mm:ss")
