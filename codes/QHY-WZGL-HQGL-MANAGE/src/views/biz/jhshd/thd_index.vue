		...
		<a-form ref="searchFormRef" name="advanced_search" :model="searchFormState" class="ant-advanced-search-form">
			<a-row :gutter="24">
				<a-col :xxl="6" :xl="6" :lg="8" :md="12" :sm="24">
					<a-form-item label="退货单号" name="shdh">
						<a-input
				...
				<a-col :xxl="6" :xl="6" :lg="8" :md="12" :sm="24">
					<a-form-item label="审核日期" name="shrq">
						<a-range-picker v-model:value="searchFormState.shrq" value-format="YYYY-MM-DD HH:mm:ss" show-time />
					</a-form-item>
				</a-col>
				<a-col :xxl="6" :xl="6" :lg="8" :md="12" :sm="24" v-show="advanced">
					<a-form-item label="供应商名称" name="gysmc">
						<a-select v-model:value="searchFormState.gysdm" placeholder="请输入发货部门">
							<a-select-option :key="item.gysdm" v-for="item in gysInfo" :value="item.gysdm">{{
								item.gysmc
							}}</a-select-option>
						</a-select>
					</a-form-item>
				...
				<a-col :xxl="6" :xl="6" :lg="8" :md="12" :sm="24" v-show="advanced">
					<a-form-item label="退货部门" name="gysmc">
						<a-tree-select
...
<script setup name="jhshd">
	...
	import mxIndex from './thdmx_index.vue'
	...
	const loadData = (parameter) => {
		...
		return cgJhShdApi.cgJhThdPage(Object.assign(parameter, searchFormParam)).then((data) => {
			return data
		})
	}
	// 重置
	...
	const print = (record) => {
		visible.value = true
		src.value = 'https://hqncg.just.edu.cn/decision/view/report?viewlet=cgjkd%252Fdjdy%252Fcgthd.cpt&id=' + record.id
	}
