...
const loadData = (parameter) => {
	...
	return cgJhSpmxApi.bzshhzPage(Object.assign(parameter, searchFormParam)).then((data) => {
		return data;
	});
};
// 重置
...
const auditBatchCgJhSpckmx = (params) => {
	spinning.value=true
	cgJhSpmxApi.auditBatchCgJhSpckmx(params).then(() => {
...
const updateGys = (record) => {
	...
	cgJhSpmxApi.updateGys( record).then((data) => {
		return data;
	});
}
...
const submit =() =>{
	spinning.value=true
	cgJhSpmxApi.auditBatchCgJhSpckmx(searchFormState).then(() => {
		spinning.value=false
		emit('successful')
		handleClose()
	})
