<template>
	<div class="setting-drawer-index-content">
		<div class="scrollbar">
			...
			<h3>表格字体大小</h3>
			<div class="snowy-setting-checkbox">
				<a-tooltip v-for="(a, i) in fontSizeList" :key="i" placement="top">
					<template #title
					><span>{{ a.tips }}</span></template
					>
					<div :class="['snowy-setting-checkbox-font', a.style]" @click="setFontSize(a.value)">Aa
						<check-outlined v-if="fontSize === a.value" class="snowy-setting-checkbox-item-select-icon" />
					</div>
				</a-tooltip>
			</div>
			<a-divider />
	...
	export default defineComponent({
		data() {
			return {
				...
				fontSize: this.$TOOL.data.get('SNOWY_fontsize') || this.$store.state.global.fontsize,
				fontSizeList: [
					{
						tips: '小字体',
						value: 'small',
		...
		methods: {
			...
			// 设置字体大小
			setFontSize(value) {
				this.$store.commit('SET_fontszie', value)
				this.$TOOL.data.set('SNOWY_fontszie', value)
				this.fontSize=value
			},
			// 切换颜色
			tagColor(value) {
